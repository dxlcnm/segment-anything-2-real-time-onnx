...
def compute_axial_cis_mat(dim: int, end_x: int, end_y: int, theta: float = 10000.0):
    """
    Compute axial cosine and sine for rotary positional encoding (real-valued, merged).
    
    Args:
        dim (int): Embedding dimension (must be even).
        end_x (int): Maximum position along x-axis (e.g., width).
        end_y (int): Maximum position along y-axis (e.g., height).
        theta (float): Frequency scaling factor, default 10000.0.
     device (str): Device for tensors, default "cuda:0".
    
    Returns:
        torch.Tensor: Merged cosine and sine tensor, shape (end_x * end_y, dim//2, 2).
                     Last dimension contains [cos_theta, sin_theta].
    """
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 4)[: (dim // 4)].float() / dim))
    t_x, t_y = init_t_xy(end_x, end_y)

    angles_x = torch.outer(t_x, freqs)  # Shape: (end_x * end_y, dim//4)
    angles_y = torch.outer(t_y, freqs)  # Shape: (end_x * end_y, dim//4)
    cos_x = torch.cos(angles_x)
    sin_x = torch.sin(angles_x)
    cos_y = torch.cos(angles_y)
    sin_y = torch.sin(angles_y)
    cos_theta = torch.cat([cos_x, cos_y], dim=-1)  # Shape: (end_x * end_y, dim//2)
    sin_theta = torch.cat([sin_x, sin_y], dim=-1)  # Shape: (end_x * end_y, dim//2)
    # Merge cos_theta and sin_theta into a single tensor
    # freqs_merged = torch.stack([cos_theta, sin_theta], dim=-1)  # Shape: (end_x * end_y, dim//2, 2)
    freqs_merged = torch.complex(cos_theta, sin_theta)
    return freqs_merged
