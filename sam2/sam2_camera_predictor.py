...
def timer_decorator(func):
    def wrapper(*args, **kwargs):
        ...
        end_time = time.time()
        logger.debug(f'{func.__name__} took {round(end_time - start_time, 3)} seconds')
        return result
    return wrapper
...
class SAM2CameraPredictor(SAM2Base):
    ...
    def track(
        ...
        if maskmem_features is not None:
            maskmem_features = maskmem_features.to(torch.float)
    ...
    def _get_image_feature(self, frame_idx, batch_size):
        ...
        if backbone_out is None:
            ...
            backbone_out = self.forward_image(image) # torch.onnx.export()
    ...
    def _run_memory_encoder(
        ...
        if self.import_onnx:
            ...
            print("maskmem_features compare: ", torch.allclose(maskmem_features.cpu(), torch.from_numpy(maskmem_feats), rtol=1e-2))
            print("maskmem_pos_enc compare: ", torch.allclose(maskmem_pos_enc[-1].cpu(), torch.from_numpy(maskmem_pos_enc_one), rtol=1e-2))
